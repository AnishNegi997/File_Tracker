...
export default function Logs() {
  ...
  useEffect(() => {
    const allMovements = getAllFileMovements();
  ...
  const getFilteredMovements = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(movement =>
        ...
        getAllFiles().find(f => f.code === movement.fileId)?.title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    if (filterDepartment) {
      filtered = filtered.filter(movement => {
        const file = getAllFiles().find(f => f.code === movement.fileId);
